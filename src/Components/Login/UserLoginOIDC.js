 ...
 console.log({requestId,idpId,loginType})
  ...
  const handleSubmit = async (e) => {
    ...
    if (loginForm.email && loginForm.password) {
      ...
      await Server.get(
        `/adaptivetoken/getRegistrationDetailsUsingEmail?email=${loginForm.email.toLowerCase()}&requestTime=${requestTime}`,
        ...
        if (response.data.resultCode === 0) {
            var jwt = response.data.resultData.jwt;
      ...
      await Server.post(
        `/user/login?email=${loginForm.email.toLowerCase()}&password=${loginForm.password}&requestTime=${requestTime}&type=${loginTypeMethod}`,
        {},
        {
  ...
  const handleOIDC = (data) => {
    const form = document.createElement("form");
            form.method = "POST";
            form.action = `https://openid.passwordless4u.com/authenticate/${requestId}/login`;
            form.innerHTML = Object.keys(data).map((key) => {
