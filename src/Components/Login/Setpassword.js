  ...
  const handleSubmit = async (e) => {
    ...
    if (passwordForm.password === passwordForm.confirmPassword) {
      ...
      if (subdomain.length >= 6) {
        const encryptPassword=bcrypt.hashSync(passwordForm.password, "$2a$10$CwTycUXWue0Thq9StjUM0u");
        await Server.post(
          `/operator/setPasswordForRegistration?password=${encryptPassword}&subDomain=${subdomain}&token=${token}&requestTime=${requestTime}`,
          {},
          {
            headers: {
