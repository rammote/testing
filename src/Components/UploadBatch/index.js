...
export default function UploadBatch() {
  ...
  const handleUsersDataImport = async () => {
    if (CSVFile) {
      ...
      data.append("tokenFile", CSVFile)
      setLoading(true)
      try {
        const res = await Server.post(
          `/token/hardwareTokenMigration?accountId=${accountId}`,
          data,
          {
            headers: {
  ...
  return (
    <div style={{ marginTop: "5rem", width: "100vw", marginLeft: "1.5rem" }}>
      <Typography variant="h5" sx={{ fontWeight: "bold", pt: 1 }}>
        {" "}
        Account Base Integration Perspective
      </Typography>
      <Typography variant="h5" sx={{ fontWeight: "bold", fontSize: "1rem" }}>
