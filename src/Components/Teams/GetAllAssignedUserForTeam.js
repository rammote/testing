...
export default function CustomizedTables() {
  ...
  React.useEffect(() => {
    ...
    Server.get(
      `/unit/getAllAssignedOperators?accountId=${accountId}&unitId=${location.state.row.unitid}&requestTime=${requestTime}`,
      {
        headers: {
  ...
  const [filterList, setFilterList] = React.useState(rows);
  useEffect(() => {
    setFilterList(rows);
  }, [rows]);
  ...
  const handleUnAssignOperator = (e, row) => {
    e.preventDefault();
    console.log("Details of User", row);
    console.log(authtoken);
    Server.get(
      `/unit/unassign?accountId=${row.accountid}&unitId=${location.state.row.unitid}&operatorId=${row.operatorid}&requestTime=${requestTime}`,
      {
        headers: {
          "content-type": "application/json",
          authToken: authtoken,
      ...
      .then((response) => {
        ...
        if (response.data.resultCode == "0") {
          Swal.fire({
            title: "Operator has Successfully UnAssigned",
            text: "",
            icon: "success",
            ...
            setRows((prevRows) =>
              prevRows.filter((item) => item.operatorid != row.operatorid)
            );
          });
        } else {
          Swal.fire({
            title: "Operator has Not UnAssigned Yet !!!",
            text: "",
            icon: "warning",
            showCancelButton: false,
            confirmButtonColor: "#3085d6",
  ...
  return (
    <>
      <div style={{ paddingTop: "90px", paddingLeft: "20px" }}>
          ...
          <Grid item xs={12} md={6}>
            ...
            >
              ...
              <div style={{ display: "grid", width: 300 }}>
                ...
                >
                  Unit Name : {location.state.row.name}
                </span>
                <span
                  style={{
                    margin: 0,
        ...
        {/* <p style={{ color: '#5C5C5C', fontSize: 25, marginLeft: 20 }}>Unit Id : {location.state.row.unitid}</p> */}
        <div
          style={{
            display: "grid",
                  ...
                  .map((row, index) => (
                    <StyledTableRow key={index}>
                      ...
                      <StyledTableCell align="center">
                        <Button
                          variant="contained"
                          onClick={(e) => handleUnAssignOperator(e, row)}
                        >
                          UnAssign
                        </Button>
                      </StyledTableCell>
