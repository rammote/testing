...
export default function RadiusPage() {
  ...
  const [appType, setAppType] = React.useState("5");
...
const [loading, setLoading] = useState(false)
  const handleSubmit = (event) => {
    ...
    Server.post(
      `/application/add?appName=${appName}&subType=${7}&appType=${appType}&accountId=${accountId}&requestTime=${requestTime}`,
      filedata,
      {
        headers: {
          "Content-Type":
      ...
      .then((response) => {
        if (response.data.resultCode === 0) {
          ...
          history.push({
            pathname: "/AxiomProtect/Applications/OpenIdApp",
            state: {
