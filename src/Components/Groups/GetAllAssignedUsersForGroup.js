...
export default function CustomizedTables() {
  ...
  React.useEffect(() => {
    Server.post(
      `/group/getAllAssignedUser?accountId=${accountId}&groupId=${location.state.row.groupid}&requestTime=${requestTime}`,
      {},
      {
        headers: {
  ...
  const handleUnAssignUser = (e, row) => {
    ...
    Server.post(
      `/group/unassign?accountId=${row.accountid}&userId=${row.userid}&requestTime=${requestTime}`,
      {},
      {
        headers: {
          "content-type": "application/json",
      ...
      .then((response) => {
        ...
        if (response.data.resultCode == "0") {
          ...
          Swal.fire({
            title: "User has Not UnAssigned Yet !!!",
            text: "",
            icon: "warning",
            showCancelButton: false,
            confirmButtonColor: "#3085d6",
          ...
          <Grid item xs={12} md={6}>
            ...
            >
              <input
                ...
                type="text"
                placeholder="Search user with email, name and phone no..."
                style={{
