    ...
    const openInNewTab1 = (url) => {
      ...
      React.useEffect(() => {
       ...
       const fetchData = async ()=>{
        await Server.get(`/sonicKYC/getVerificationData?userId=${location.state.row.userid}&appId=${appId}&requestTime=${requestTime}`, {
          headers: {
              'content-type': 'application/json',
              authToken: authtoken,
              "Access-Control-Allow-Origin": "*",
  ...
  <h4 style={{color: '#0D4990', borderColor: '#0D4990'}}>Download app from app store and scan the QR code.</h4>
        ...
        <Alert  sx={{ width: 'fit-content' }} icon={false}>
            <div style={downloadContainerStyle}>
              <a href='https://play.google.com/store/apps/details?id=com.sonickycv2' target="_blank" rel="noopner noreferrer">
                <img src={googlePlayStore} alt="googlePlayStore" width="120" />
              </a>

              <a href='https://apps.apple.com/us/app/sonickyc/id1572167364' target="_blank" rel="noopner noreferrer">
