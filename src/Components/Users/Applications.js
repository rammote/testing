  ...
  const handleOnClick = e => {
    ...
    const handleChangePage = (event, newPage) =>  setPage(newPage);
      ...
      const handleStatusChange = (e, row) => {
        ...
        Server.get(`/application/setStatus?accountId=${row.accountid}&appId=${row.appid}&status=${status}&requestTime=${requestTime}`,{
          headers: {
              'content-type': 'application/json',
               authToken: authtoken,
              "Access-Control-Allow-Origin": "*",
      ...
      const handleSubmit = (e,row) => {
        ...
        Server.get(`/user/unassign?accountId=${accountId}&userId=${location.state.row.userid}&appId=${row.appid}&requestTime=${requestTime}`, {
        ...
        if(response.data.resultCode == 0  || response.data.resultMessage == "Success"){
            ...
            Swal.fire(
                'User has successfully UnAssigned!',
                'Your User has UnAssigned Successfully.',
                'success'
                )
