import * as React from 'react';
import { DataGrid,GridToolbarContainer,
    GridToolbarExport,
    gridClasses, } from '@mui/x-data-grid';
function CustomToolbar() {
    return (
      <GridToolbarContainer className={gridClasses.toolbarContainer}>
        <GridToolbarExport
          csvOptions={{
            fields: [
              "strSerialNo",
              "strManufacturer",
              "strTimeInterval",
              "strMAC",
              "MAC",
              "strEncAlgo",
              "strOtpalgo",
            ],
          }}
          printOptions={{
            fields: [ 
              "strSerialNo",
              "strManufacturer",
              "strTimeInterval",
              "strMAC",
              "MAC",
              "strEncAlgo",
              "strOtpalgo",
            ],
          }}
          style={{ margin: "0px 10px", fontSize: "16px" }}
        />
      </GridToolbarContainer>
    );
  }
const columns = [
    { field: 'strSerialNo', headerName: 'Serial No.', minWidth: 150, flex: 1 },
    { field: 'strManufacturer', headerName: 'Manufacture', minWidth: 130, flex: 1 },
    { field: 'strTimeInterval', headerName: 'Time Interval', minWidth: 130, flex: 1 },

    {
        field: 'strMAC',
        headerName: 'MAC',
        minWidth: 90,
        flex: 1
    },
    {
        field: 'strEncAlgo',
        headerName: 'Enc Algo',
        sortable: true,
        minWidth: 160,
        flex: 1
    },
    {
        field: 'strOtpalgo',
        headerName: 'OTP Algo',
        sortable: true,
        minWidth: 160,
        flex: 1
    },
];


export default function InvalidTokensTable({ failedOTPEntriesData }) {
    return (
        <div style={{ maxHeight: 400, width: '80%' }}>
            {failedOTPEntriesData && failedOTPEntriesData.length > 0 && <DataGrid
                rows={failedOTPEntriesData ?? []}
                columns={columns}
                getRowId={(row) => row?.strSerialNo}
                pageSize={5}
                rowsPerPageOptions={[5]}
                components={{
                    Toolbar: CustomToolbar,
                  }}
            />
            }
        </div>
    );
}
